...
if __name__ == '__main__':
    ...
    def _detect_tags(gray):
        """Detect AprilTags; return (corners, ids) in cv2.aruco layout.

        pupil_apriltags corner order: BL, BR, TR, TL
        cv2.aruco corner order:       TL, TR, BR, BL  → reindex [3,2,1,0]

        Filters: hamming == 0 (no bit errors), decision_margin >= threshold,
        and only known tag IDs (0-3).
        """
        ...
        filtered = [d for d in dets
                    if d.hamming == 0
                    and d.decision_margin >= MIN_DECISION_MARGIN
                    and d.tag_id in VALID_TAG_IDS]
        if not filtered:
            return [], None
        corners = [d.corners[[3, 2, 1, 0]].reshape(1, 4, 2).astype(np.float32)
                   for d in filtered]
        ids = np.array([[d.tag_id] for d in filtered], dtype=np.int32)
        return corners, ids

    def _draw_tags(img, corners, ids):
        for i, corn in enumerate(corners):
            pts = corn.reshape(4, 2).astype(np.int32)
    ...
    calib_poses = []   # (R 3×3, tvec 3×1) pairs from per-frame PnP
    calib_start = None
    ...
    try:
        while True:
            ...
            if state == State.CALIBRATING:
                ...
                if ids is not None:
                    _draw_tags(frame, corners, ids)
                    # Only use frames where all 4 tags are visible (16 corners)
                    if len(corners) == 4:
                        ok_p, R_p, tv_p = solve_plane_pose(corners, ids, K, D_CALIB)
                        if ok_p:
                            calib_poses.append((R_p, tv_p))
                ...
                if elapsed >= CALIB_DURATION:
                    if n_poses > 0:
                        # Average rotations (matrix mean + SVD re-orthogonalisation)
                        R_avg = np.mean([R for R, _ in calib_poses], axis=0)
                        U, _, Vt = np.linalg.svd(R_avg)
                        frozen_R = U @ Vt
                        frozen_tvec = np.mean([tv for _, tv in calib_poses], axis=0)
                        print(f'[CALIB] LOCKED — averaged {n_poses} valid poses')
                    else:
                        frozen_R, frozen_tvec = default_pose()
                        print('[CALIB] No valid poses found — using default pose')
            ...
            # ── Grid overlay (crop to current frame size) ──
            fh, fw = frame.shape[:2]
            if calib_grid_overlay is not None and calib_grid_mask_img is not None:
                _co = calib_grid_overlay[:fh, :fw]
                _cm = calib_grid_mask_img[:fh, :fw]
                np.copyto(frame, _co, where=_cm[:, :, None] > 0)
            if grid_overlay is not None and grid_mask_img is not None:
                _go = grid_overlay[:fh, :fw]
                _gm = grid_mask_img[:fh, :fw]
                np.copyto(frame, _go, where=_gm[:, :, None] > 0)
